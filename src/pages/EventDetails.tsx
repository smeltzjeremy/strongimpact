...
// 📊 DYNAMIC SCHEMAS: Prepared for absolute administrative database control
...
interface SpecialGuest {
  id: number;
  name: string;
  background: string; // e.g. "Collegiate Athlete" or "NFL Pro"
  bio: string;
  imageUrl: string;
  metric: string;
}
...
export default function EventDetails(): React.JSX.Element {
  ...
  // TRACKING ENGINE STATE: Manages calendar rendering selections
  const [currentYear] = useState<number>(2026);
  const [currentMonth] = useState<number>(5); // June (0-Indexed tracking structure)
  const [selectedEvent, setSelectedEvent] = useState<CalendarEvent | null>(null);

  // 🛠️ ADMIN DATA BLOCK: Swap these hardcoded items out with live database hooks later
  const promotionalFlyer: PromotionalFlyer = {
    ...
    imageUrl: "", // Left blank intentionally to demonstrate the skeuomorphic photo fallback slot
  ...
  // ADMIN-CONTROLLED REGISTRATION PORTAL LINKS (Change URLs instantly here)
  const registrationLink: RegistrationLink = {
    label: "Looking to lock in your roster spot?",
    url: "https://forms.google.com" // Swap with your actual Google Form, Linktree, or Jotform URL link
  };

  // 🗓️ NATIVE CALENDAR ENGINE LOGIC
  ...
  return (
    <div className="min-h-screen bg-[#040406] text-white flex flex-col items-center justify-between p-4 sm:p-6 overflow-y-auto selection:bg-red-500/30 font-sans relative select-none">
      ...
      {/* 🔮 MULTI-LAYERED GLOW ENGINE (Brought across directly from your template) */}
      ...
      {/* TOP HEADER CONTROLS INTERACTIVE LAYER */}
      ...
      <div className="w-full max-w-xl flex flex-col justify-center items-center z-10 py-6 space-y-8">
        ...
        {/* 1. FLYER SHOWCASE MODULE (Card Type 1 Overlay Mapping) */}
        <div className="w-full p-5 bg-gradient-to-b from-zinc-900/40 via-zinc-950/60 to-zinc-950/90 border border-white/10 rounded-[28px] backdrop-blur-3xl backdrop-saturate-[180%] shadow-[0_30px_60px_-15px_rgba(0,0,0,0.8),inset_0_1px_1px_rgba(255,255,255,0.15)] transition-all duration-500 hover:-translate-y-1 group relative overflow-hidden text-left">
          ...
          {/* CINEMATIC WIDE FLYER COMPONENT SLOT */}
          ...
          <a 
            ...
            target="_blank"
            rel="noreferrer"
            className="w-full block text-center py-3 bg-white text-black font-black text-[10px] uppercase tracking-widest rounded-xl transition-all duration-300 hover:bg-zinc-200 active:scale-[0.98] shadow-xl shadow-black/40"
          >
            {promotionalFlyer.actionText}
          </a>
        </div>

        {/* 2. DYNAMIC CALENDAR INTERACTIVE ENGINE */}
        <div className="w-full p-5 bg-zinc-900/20 border border-white/10 rounded-[28px] backdrop-blur-3xl shadow-2xl text-left">
            ...
            {Array.from({ length: calendarRows }).map((_, rowIndex) => (
              <div key={rowIndex} className="grid grid-cols-7 gap-1.5">
                {Array.from({ length: 7 }).map((_, colIndex) => {
                  ...
                  const formattedDayStr = dayNumber < 10 ? `0${dayNumber}` : `${dayNumber}`;
                  const formattedMonthStr = currentMonth + 1 < 10 ? `0${currentMonth + 1}` : `${currentMonth + 1}`;
                  const fullDateKey = `${currentYear}-${formattedMonthStr}-${formattedDayStr}`;
          ...
          {/* Dynamic Event Metadata View Drawer Expansion */}
          {selectedEvent ? (
        ...
        {/* 3. SPECIAL GUESTS DECK (Card Type 2 Sub-Grid Panel Mapping) */}
        <div className="w-full space-y-3">
          ...
          <div className="grid grid-cols-2 gap-3">
            {specialGuests.map((guest, index) => (
              <div 
                key={index}
                className="p-4 bg-zinc-900/30 border border-white/5 rounded-2xl backdrop-blur-2xl shadow-[0_20px_40px_rgba(0,0,0,0.6)] flex flex-col justify-between text-left h-[175px] transition-all duration-500 hover:-translate-y-1 hover:border-white/10 group/guest"
              >
                <div>
                  <div className="flex justify-between items-center mb-3">
                    {/* SKEUOMORPHIC PROFILE CARD SLOT */}
                    <div className="w-9 h-9 rounded-xl bg-zinc-950 border border-white/10 flex items-center justify-center overflow-hidden shadow-inner shrink-0">
                      {guest.imageUrl ? (
                        ...
                        <span className="text-xs opacity-40 group-hover/guest:opacity-70 transition-opacity">👤</span>
                      )}
                    </div>
                    <span className="text-[7px] uppercase tracking-widest font-black px-2 py-0.5 rounded bg-zinc-950 border border-white/5 text-zinc-400 group-hover/guest:text-red-400 group-hover/guest:border-red-500/20 transition-colors">{guest.metric}</span>
                  </div>
        ...
        {/* 4. REGISTRATION HUB MODULE (Card Type 3 Search Pill Mapping) */}
        <div className="w-full p-3.5 bg-zinc-950/60 border border-white/5 rounded-2xl backdrop-blur-2xl shadow-[inset_0_1px_2px_rgba(0,0,0,0.6)] flex items-center justify-between transition-all hover:border-white/10 text-left">
          <div className="flex items-center space-x-3 min-w-0">
            ...
            <span className="text-xs font-bold text-zinc-400 truncate tracking-wide">
          ...
          <a 
            ...
            target="_blank"
            rel="noreferrer"
            className="px-3 py-1 bg-white/5 border border-white/10 rounded-lg text-[9px] uppercase font-black tracking-widest text-zinc-300 cursor-pointer hover:bg-white/10 transition-all text-center whitespace-nowrap"
          >
            Register Now
          </a>
        </div>
