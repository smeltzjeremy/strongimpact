...
import { useNavigate, Link } from 'react-router-dom'; // ← Correct library reference path fixed
  ...
  const hubs: BentoHub[] = [
    {
      ...
      description: "Sign-up forms to support camps, events, and mentorship tracks.",
      path: "/get-involved/volunteer",
      gridSpan: "col-span-1 h-[130px]",
      brandColor: "bg-gradient-to-tr from-blue-600 to-cyan-600",
      glowColor: "rgba(59, 130, 246, 0.2)",
      icon: "🙋‍♂️"
    },
    {
      name: "Sponsorship Tiers",
      description: "Corporate and local business packages (Bronze to Platinum).",
      path: "/get-involved/sponsors",
      gridSpan: "col-span-1 h-[130px]",
      brandColor: "bg-gradient-to-tr from-amber-500 to-yellow-600",
      glowColor: "rgba(245, 158, 11, 0.2)",
      icon: "💎"
    },
    {
      name: "Donation Processor",
      description: "One-time gifts, recurring monthly options, or sponsor a child.",
  ...
  return (
    <div className="fixed inset-0 bg-[#030306] z-10 flex flex-col items-center justify-between p-4 sm:p-6 overflow-y-auto select-none perspective-[1200px]">
      <div className="fixed inset-0 pointer-events-none overflow-hidden z-0">
        <div className="absolute top-[10%] right-[30%] w-[70vw] h-[70vw] bg-red-600/10 rounded-full blur-[120px]" />
      ...
      <div className="w-full max-w-md flex justify-between items-center mt-2 z-10">
        ...
        <span className="text-[9px] uppercase font-black tracking-widest text-zinc-500 opacity-60">Stepping Stone 05</span>
      </div>

      <div className="w-full max-w-md flex flex-col justify-center items-center z-10 py-6">
        <div className="text-center mb-8">
          <h1 className="text-4xl font-black uppercase tracking-tighter text-transparent bg-clip-text bg-gradient-to-b from-white to-zinc-400">GET <span className="text-transparent bg-clip-text bg-gradient-to-r from-red-500 to-rose-600">INVOLVED</span></h1>
          <p className="text-[8px] uppercase tracking-widest text-zinc-400 font-black mt-3 bg-white/[0.02] border border-white/10 px-4 py-2 rounded-full inline-block backdrop-blur-2xl">Support The Mission</p>
        </div>
          ...
          {hubs.map((hub, index) => {
            ...
            return (
              <Link 
                ...
                ref={cardRef} 
                to={hub.path} 
                className={`${hub.gridSpan} block p-5 bg-white/[0.02] hover:bg-white/[0.05] rounded-2xl backdrop-blur-2xl backdrop-saturate-[180%] transition-all duration-200 ease-out relative overflow-hidden flex flex-col justify-between group border border-white/10`}
                style={{
